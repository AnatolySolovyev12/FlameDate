			...
			if (jsonDoc["ok"].toBool())
			{
				...
				for (const QJsonValue& value : updates)
				{
					...
					if (messageObj.contains("text") && fromObj.contains("id")) {
						...
						qDebug() << "Received message (" << value["update_id"].toInt() << "): " << messageObj["text"].toString() << "chatId: " << fromObj["id"].toInt();

						iD = value["update_id"].toInt() + 1;

						if (text == "/start")
						{
							const QString tempt = QString::number(fromObj["id"].toInt()) + "@" + QString::number(fromObj["id"].toInt());
							startBool = true;
...
void TelegramJacket::sendMessage(const QString message, const QString d_message)
{
	...
	for (auto& val : message)
	{
		...
		if (val == '@')
		{
			idMassive.push_back(temporary);
			temporary = message;
			temporary = temporary.mid(message.indexOf('@') + 1);
			break;
		}
